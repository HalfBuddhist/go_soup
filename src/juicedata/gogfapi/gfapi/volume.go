...
func (v *Volume) Chtimes(name string, mtime time.Time) error {
	...
	amtime[1].tv_sec = C.long(mtime.Unix())
	amtime[1].tv_nsec = C.long(mtime.Nanosecond())
...
// Lstat returns an os.FileInfo object describing the named file.
// It doesn't follow the link if the file is a symlink.
//
// Returns an error on failure
...
// get xattr for a path
func (v *Volume) ListXattr(path string, value []byte, size uint64) (
	uint64, error) {

	cpath := C.CString(path)
	defer C.free(unsafe.Pointer(cpath))

	ret, err := C.glfs_listxattr(
		v.fs, cpath, unsafe.Pointer(&value[0]), C.size_t(size))
	if err != nil {
		return 0, err
	}
